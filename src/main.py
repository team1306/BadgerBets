from appwrite.client import Client
from appwrite.services.users import Users
from appwrite.services.databases import Databases
from appwrite.exception import AppwriteException
import os

# This Appwrite function will be executed every time your function is triggered
def main(context, userId, amount):
    # You can use the Appwrite SDK to interact with other services
    # For this example, we're using the Users service
    client = (
        Client()
        .set_endpoint('https://cloud.appwrite.io/v1')
        .set_project('67609b010021900fc6e6')
        .set_key('standard_')
    )
    users = Users(client)
    databases = Databases(client)
    if(context == "get"):
        getBucks(userId)
    elif(context == "set"):
        setBucks(userId, amount)
    elif(context == "create"):
        createBucks(userId, amount)

def getBucks(userId):
    client = (
        Client()
        .set_endpoint('https://cloud.appwrite.io/v1')
        .set_project('67609b010021900fc6e6')
        .set_key('standard_')
    )
    databases = Databases(client)
    result = databases.get_document('6760b9c20030df251f1c','badgerBucks', userId)
    return result
def setBucks(userId, amount):
    client = (
        Client()
        .set_endpoint('https://cloud.appwrite.io/v1')
        .set_project('67609b010021900fc6e6')
        .set_key('standard_')
    )
    databases = Databases(client)
    document.update_document('6760b9c20030df251f1c','badgerBucks', userId, {'username': userId, 'badgerBucks': amount})

    

def createBucks(userId,amount):
     client = (
        Client()
        .set_endpoint('https://cloud.appwrite.io/v1')
        .set_project('67609b010021900fc6e6')
        .set_key('standard_')
    )
    databases = Databases(client)
    databases.create_document('6760b9c20030df251f1c','badgerBucks', userId, {'username': userId, 'badgerBucks': amount})

    




        
